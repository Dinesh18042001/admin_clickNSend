...
const Subscription = () => {
  ...
  const fetchdata = async (type = "customer") => {
    setLoadingCard(true);
    try {
      const response = await apiAdminConfig.get(`/api/auth/master/plan/list/${type}/''`);
      if (response.status === 200) {
        setLoadingCard(false);
        setData(response?.data.view_data);
      }
  ...
  React.useEffect(() => {
    fetchdata(); // Default fetch for customer
  }, []);
  
  // Example of fetching data for a different type
  const handleFetchForDriver = () => {
    fetchdata('driver');
  };
  
  const handleFetchForCompany = () => {
    fetchdata('company');
  };
